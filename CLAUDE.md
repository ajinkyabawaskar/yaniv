...
Other in-memory machinery inside `GameStateController` worth knowing about: a `ScheduledExecutorService` drives Yaniv contest timers **and per-turn timers** — when a player's turn timer (`game.turn-timer-seconds`) expires, `AutoPlayStrategy` plays the best move on their behalf (`finishMutation` is the single post-mutation hook: persist snapshot → bookkeeping → schedule next timer → broadcast). Full engine state is snapshotted to Redis after every mutation (`game:{id}:state` via `GameService.saveGameState`, serialized by `game/GameSnapshot`), so a restarted server restores games instead of re-dealing; a lost snapshot aborts the room back to LOBBY. Action IDs are deduplicated per player, and disconnect/reconnect handling keeps disconnected players "in" an active game (`SessionDisconnectEvent`/`SessionConnectedEvent` listeners) until the game ends.
